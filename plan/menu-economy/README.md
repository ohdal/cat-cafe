...
| `STARTING_GOLD` | **1,000** | 신규 시작 골드(초반 페이싱용). `useGameStore.currency` 초기값 |
| `MAX_OFFLINE_HOURS` | **8** | 오프라인 정산 상한(시간) |
| `ACCRUAL_INTERVAL_MS` | **1000** | 적립 tick 간격(1초) |

### 메뉴 기본 수익(1시간당, L0 · 해금 순서대로 상승) + 해금 층

값은 **L0(기본) 수익**이며, 최대 레벨(L10)에서 **약 ×20**(≈ base×20)이 천장. 초반 체감이 너무 낮다는
피드백으로 **아메리카노 기준 10 → 30(×3)** 로 카탈로그 전체를 스케일했다(가장 싼 메뉴 30 → 천장 ≈ 603).

> **⚠ 추가 스케일(최신)**: 그래도 가구·꾸미기 사려고 모으다 보면 너무 안 벌린다는 피드백으로
> `src/features/menu/catalog.ts`의 baseIncomePerHour에 **×20을 한 번 더** 적용했다(아메리카노
> 30 → **600**, 1F 5메뉴 L0 합 192 → **3,840 G/h ≈ 1.07 G/s** — 초반부터 초당 1골드 이상 체감).
> 업그레이드 비용은 이 배율의 영향을 받지 않도록 `formulas.ts`의 `UPGRADE_COST_FACTOR`를 같은
> 비율로 낮춰 상쇄했다(2 → 0.1) — 업그레이드 비용 표는 여전히 유효하다. 바로 아래 두 표(메뉴
> 기본 수익 표 · 수익/비용 곡선 표)는 새 ×20 값으로 갱신했지만, **더 아래
> [진행 시뮬레이션 & 밸런스 분석](#진행-시뮬레이션--밸런스-분석) 절의 Gold/Hour·도달 시간 등은
> 아직 옛 수치**(×20 반영 전)라 실제보다 낮게 나온다 — 재계산 전까지는 그 절의 상대적 성장
> 배율(레벨당 ×1.7~1.9 등)만 참고할 것.

| 층(해금 카페Lv) | 메뉴 (baseIncomePerHour, L0) |
| --- | --- |
| 1F (Lv1) | 아메리카노 600 · 카페라떼 720 · 바닐라 라떼 840 · 고양이 간식 600 · 고양이 장난감 1,080 |
| 2F (Lv2) | 자스민티 1,080 · 캐모마일티 1,200 · 얼그레이티 1,320 |
| 3F (Lv3) | 레몬 에이드 1,560 · 자몽 에이드 1,680 · 청포도 에이드 1,800 |
| 4F (Lv4) | 초코 라떼 2,160 · 말차 라떼 2,400 · 아이스티 2,640 |
| 5F (Lv5) | 크림 아인슈페너 3,120 · 솔티드 카라멜 라떼 3,480 · 헤이즐넛 모카 3,840 |
| 6F (Lv6) | 냥카라멜 프라페 4,440 · 냥자바칩 프라페 4,920 · 냥말차 프라페 5,400 |
| 7F (Lv7) | 딸기 프라페 6,300 · 발바닥 쿠키 프라페 7,080 · 우주냥 티라미수 프라페 7,800 |

### 메뉴 수익/업그레이드 비용 곡선 (예: 아메리카노 base=600)

`menuIncome = round(base × 1.35^level)`, `upgradeCost = round(base × 0.1 × 1.6^level)`
(업그레이드 비용 공식은 UPGRADE_COST_FACTOR를 2→0.1로 낮춰 base ×20과 상쇄했으므로,
아래 비용 값 자체는 ×20 적용 전과 동일하다.)
...
| --- | --- | --- | --- | --- | --- |
| 1 | 5 | 192 | ×1.0 | **192** | ≈0.053 |
| 2 | 8 | 372 | ×1.5 | **558** | ≈0.155 |
| 3 | 11 | 624 | ×2.0 | **1,248** | ≈0.347 |
| 4 | 14 | 984 | ×2.5 | **2,460** | ≈0.683 |
| 5 | 17 | 1,506 | ×3.0 | **4,518** | ≈1.255 |
| 6 | 20 | 2,244 | ×3.5 | **7,854** | ≈2.182 |
| 7 | 23 | 3,303 | ×4.0 | **13,212** | ≈3.670 |

> 위는 **전 메뉴 L0** 기준(아메리카노 base=30 스케일 적용 후). 메뉴를 레벨업하면 최대 ×20까지
> 커진다(예: Lv7 + 전 메뉴 L10 → 3,303×20 ≈ 66,060 × 배수 4.0 ≈ **265,656 G/h**, 아래
> [밸런스 분석](#진행-시뮬레이션--밸런스-분석) 참고).

> **⚠ 페이싱 메모**: 아메리카노 기준 base=10은 체감상 너무 낮다는 피드백으로 **×3 스케일(→30)** 을
> 채택. 시작 골드는 `STARTING_GOLD`**=1,000**(채택) — 첫 업그레이드 즉시, 첫 확장(Lv2, 5,000골드)까지
> **약 13시간**(24h 연속 방치 기준). 시작 골드는 초반 벽만 없앨 뿐 Lv7 도달(~3.8일)·풀업(~3.9일)엔
> 영향이 작다(총비용 610만 대비 미미) — 아래 [⚠ 페이싱 표](#-페이싱--초반-지연--시작-골드로-완화) 참고.

---

## 진행 시뮬레이션 & 밸런스 분석

> 📊 **시각화**: [`analysis.html`](analysis.html) — 아메리카노 base=10 시절 그래프라 지금은 수치가
> 어긋나 있다. 재생성 전까지는 아래 표/막대를 정본으로 본다.
>
> 위 상수/수치를 그대로 넣고 시뮬레이션한 결과. 가정: **그리디(ROI 최적) 전략 + 24시간 연속 방치,
> 확장은 골드 비용만으로 게이팅**(설계 결정 — 아래 설계 노트 참고). 메뉴 base 수익 ×3 스케일
> (아메리카노 10→30) + 시작 골드 `STARTING_GOLD`=1,000 적용 후 재계산.

### 확정 수치 (계산값)

| 항목 | 값 |
| --- | --- |
| **최대 수익** (Lv7 + 전 메뉴 L10) | **265,656 G/h ≈ 73.8 골드/초** |
| 전 메뉴 L0→L10 업그레이드 총비용 | 1,199,553 gold |
| 카페 Lv1→7 확장 총비용 | 4,905,000 gold (메뉴 스케일과 무관 — 골드 비용만 게이팅이라 불변) |
| **풀업까지 필요한 총 골드** | **약 610만 gold** (확장이 약 80%, 메뉴 업그레이드가 약 20%) |

### 카페 레벨별 최대 수익 (전 메뉴 L10 기준)

```
Lv1 ▊                                                      3,860 G/h  (1.07/s)
Lv2 ██▎                                                    11,219 G/h  (3.12/s)
Lv3 █████▏                                                 25,092 G/h  (6.97/s)
Lv4 ██████████                                             49,463 G/h (13.74/s)
Lv5 ██████████████████▌                                    90,843 G/h (25.23/s)
Lv6 ████████████████████████████████▏                     157,920 G/h (43.87/s)
Lv7 ██████████████████████████████████████████████████████ 265,656 G/h (73.79/s)
```

레벨당 대략 **×1.7~1.9** 성장(등급배수 +0.5 + 신메뉴 해금 복합) — ×3 스케일 이후에도 성장 배율 자체는
그대로 유지된다(모든 항이 base에 비례하므로 배율은 스케일과 무관).

### 7층 도달까지 걸리는 시간 (누적, 24h 방치 · 시작골드 1,000)
...
> 실제론 **오프라인 상한(`MAX_OFFLINE_HOURS`=8)** 때문에 하루 8~16h만 적립되면 체감 **약 5.9~11.7일**.
> (시작골드별 비교는 아래 ⚠ 페이싱 표 참고.)

### ⚠ 페이싱 — 초반 지연 → 시작 골드로 완화

무보정(시작 골드 0) 시 시작 0.053 G/s → 첫 업그레이드(60골드) ~19분, 첫 확장(Lv2)까지 ~17시간으로
초반이 다소 답답하다. **시작 골드 `STARTING_GOLD`=1,000** 지급으로 완화(채택). 시작 골드별 재시뮬:

| 시작 골드 | 첫 업글 | 첫 확장(Lv2) | Lv4 | Lv7 | 풀업 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 19분 | 16.9시간 | 2.0일 | 4.0일 | 4.0일 |
| **1,000 (채택)** | **즉시** | **13.0시간** | 1.8일 | 3.8일 | 3.9일 |
| 2,000 | 즉시 | 10.5시간 | 1.7일 | 3.7일 | 3.8일 |
| 5,000 | 즉시 | 4.8시간 | 1.4일 | 3.5일 | 3.5일 |
| 10,000 | 즉시 | 즉시 | 1.2일 | 3.2일 | 3.3일 |
| 50,000 | 즉시 | 즉시 | 15.6시간 | 2.7일 | 2.7일 |

- 1,000은 **첫 업그레이드만 즉시 열어주고 첫 확장(5,000골드)까지는 여전히 기다리게** 한다 — 손맛은
  주되 확장은 방치로 도달하게 하려는 의도(5,000을 그대로 주면 첫 확장까지 김빠짐).
- 10,000+ 부터는 시작하자마자 확장이 열려 초반 성장감이 사라짐.
- **시작 골드는 초반 벽만 없앤다.** Lv7 도달은 1,000이어도 3.8일로 무보정(4.0일)과 큰 차이 없음
  (풀업 총비용 610만 대비 미미). 전체 그라인드를 줄이려면 기본 수익/비용/표시단위 등 별도 레버 필요.
...
export const DRINKS: MenuItem[] = [
  ...
  // 2F(54/60/66) … 7F(315/354/390) — 전체 표는 위 "메뉴 기본 수익" 참고
];
...
- [ ] 시작 수치 검증: 카페 Lv.1, 5메뉴 전부 L0 → 약 **3,840 G/h ≈ 1.07 G/s** (×20 스케일 반영 후)
